...
STATE_CATEGORY = {
    ...
    "METADATA_REJECTED": "action-required",
    "INVALID_BINARY": "action-required",
...
def cycle(version, build):
    if not VERSION_RE.fullmatch(version or "") or not BUILD_RE.fullmatch(build or ""):
        raise MonitorError(
            "Cycle must use a numeric marketing version and build number"
        )
...
def observation(version, build, state):
    ...
    return {
        "cycle": cycle(version, build),
        "state": state,
        "category": STATE_CATEGORY.get(state, "unknown"),
    }
...
def page_payloads(fetch, url, validate=validate_asc_url):
    seen = set()
    while url:
        validate(url)
        if url in seen:
            raise MonitorError("App Store Connect pagination loop")
        seen.add(url)
        payload = fetch(url)
        data = payload.get("data") if isinstance(payload, dict) else None
        if not isinstance(data, list):
            raise MonitorError("App Store Connect returned malformed collection")
        yield payload
        nxt = (
...
def validate_version_url(url, version):
    validate_asc_url(url)
    parsed = urllib.parse.urlsplit(url)
    if parsed.path != f"/v1/apps/{APP_ID}/appStoreVersions" or parsed.fragment:
        raise MonitorError("App Store Connect returned unsafe pagination URL")
    try:
        query = urllib.parse.parse_qs(parsed.query, strict_parsing=True)
    except ValueError:
        raise MonitorError("App Store Connect returned unsafe pagination URL")
    expected = urllib.parse.parse_qs(urllib.parse.urlsplit(version_url(version)).query)
    cursor = query.pop("cursor", None)
    if query != expected or (
        cursor is not None and (len(cursor) != 1 or not cursor[0])
    ):
        raise MonitorError("App Store Connect returned unsafe pagination URL")
    return url
...
def validate_scope(scope, version):
    if (
        ...
        or scope != token_scope(version)
        or any(not value.startswith("GET /") for value in scope)
    ):
...
def resolve(fetch, version, build):
    ...
    for payload in page_payloads(
        fetch, version_url(version), lambda url: validate_version_url(url, version)
    ):
    ...
    if len(versions) != 1:
        raise MonitorError(
            "Exact marketing version is absent, ambiguous, or superseded"
        )
    _, attributes = attr(versions[0], "app store version")
    relationships = versions[0].get("relationships")
    linkage = (
        relationships.get("build", {}).get("data")
        if isinstance(relationships, dict)
        else None
    )
    if (
        not isinstance(linkage, dict)
        or linkage.get("type") != "builds"
        or not isinstance(linkage.get("id"), str)
    ):
        raise MonitorError("Exact version has no bound build")
    build_attrs = builds.get(linkage["id"])
    if (
        build_attrs is None
        or build_attrs.get("version") != build
        or build_attrs.get("expired") is not False
    ):
        raise MonitorError("Exact build is absent, mismatched, expired, or superseded")
    return observation(
        target["version"], target["build"], attributes.get("appVersionState")
    )
...
def state_document(entries):
    return (
        "<!-- asc-review-monitor-state "
        + json.dumps({"v": 1, "cycles": entries}, separators=(",", ":"), sort_keys=True)
        + " -->"
    )
...
def parse_state(body):
    ...
    try:
        ...
        return []
    rows = (
        value.get("cycles") if isinstance(value, dict) and value.get("v") == 1 else None
    )
    ...
    for row in rows:
        ...
        try:
            ...
            if isinstance(status, str) and status in SAFE_STATES | {"UNKNOWN"}:
                valid.append(
                    {"version": c["version"], "build": c["build"], "status": status}
                )
        except MonitorError:
            pass
...
def jwt(version):
    key_id, private = (
        os.environ.get(x, "")
        for x in ("ASC_REVIEW_MONITOR_KEY_ID", "ASC_REVIEW_MONITOR_PRIVATE_KEY")
    )
    if not all((key_id, private)):
        raise MonitorError("Review monitor credential is not configured")
    ...
    try:
        ...
        raise MonitorError("Review monitor credential could not sign a request")
    finally:
...
def asc_fetch(token):
    ...
    def fetch(url):
        ...
        try:
            ...
            raise MonitorError("App Store Connect read request failed")
...
def gh(method, path, payload=None):
    ...
    if not token:
        raise MonitorError("GitHub notification token is unavailable")
...
def find_cycle_issue(target):
    ...
    if not re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", repo):
        raise MonitorError("GitHub repository identity is invalid")
    issues, page = [], 1
    while True:
        batch = gh("GET", f"/repos/{repo}/issues?state=all&per_page=100&page={page}")
        if not isinstance(batch, list):
            raise MonitorError("GitHub returned malformed issue collection")
        issues.extend(batch)
        if len(batch) < 100:
            break
        page += 1
    ...
    matches = [
        x
        for x in issues
        if isinstance(x, dict) and x.get("title") == title and "pull_request" not in x
    ]
    if len(matches) > 1:
        raise MonitorError("Notification state issue is ambiguous")
...
def notify(obs, rearm, event_name, located=None):
    ...
    if issue is not None and issue.get("state") == "closed" and not rearm:
        return "disabled"
    entries, changed = update_dedup(
        parse_state(issue.get("body") if issue else ""), obs, rearm
    )
...
def main():
    ...
    try:
        ...
        if (
            located[1] is not None
            and located[1].get("state") == "closed"
            and not args.rearm
        ):
            print(
                f"ASC_REVIEW_MONITOR version={target['version']} build={target['build']} notification=disabled"
            )
            return 0
        obs = resolve(asc_fetch(jwt(args.version)), args.version, args.build)
        outcome = notify(
            obs, args.rearm, os.environ.get("GITHUB_EVENT_NAME", ""), located
        )
        print(
            f"ASC_REVIEW_MONITOR version={obs['cycle']['version']} build={obs['cycle']['build']} category={obs['category']} state={obs['state']} notification={outcome}"
        )
    except MonitorError as e:
        print("ASC_REVIEW_MONITOR " + redact(e), file=sys.stderr)
        return 1
    except Exception:
        # Do not turn HTTP bodies, headers, or tracebacks into runner output.
        print("ASC_REVIEW_MONITOR unexpected monitor failure", file=sys.stderr)
        return 1
    return 0
