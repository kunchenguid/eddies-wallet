...
The app is kid-first: the child's read-only wallet is the root, and parent access is a transient in-memory elevation (`WalletStore.elevation`) that must never be persisted or survive backgrounding. `docs/product-requirements.md` owns split-audience money language, external-brand placement, child-personal everyday chrome, Parent terminology, and excluded product surfaces; brand and profile-derived runtime strings live in `ProductBrand` and `ChildProfileCopy`. Parents edit the child nickname only inside the Parent area (`EditChildProfileView` / `WalletStore.updateChildProfile` via `PUT /v1/child`); the kid home never exposes profile editing. Parent action buttons use `ActionButton`/`ActionButtonMetrics` with one continuous rounded geometry for fill and hit target - do not reintroduce `.buttonStyle(.bordered)` behind a mismatched card background. Signed-in simulator and UI-test states use the Debug-only launch seam in `EddysWallet/DebugScenarios.swift`; that file owns its supported environment variables, and tests must never use real accounts. Follow the evidence-directory environment contract documented on `EvidenceCaptureUITests` when capturing labeled screenshots. `EddysWallet.xcodeproj/project.pbxproj` is hand-maintained with deterministic hex IDs; follow the existing ID conventions when adding files or targets.
