...
- Ordinary pull requests (including forks) validate through `.github/workflows/ci.yml`, which references no secrets and runs with `contents: read`. Its `xcode` job builds and tests the Debug configuration, then compiles the Release configuration for generic iOS with signing disabled so Debug-only source cannot wait until a tag-scoped archive to fail. No `pull_request_target` trigger exists in this repository.
