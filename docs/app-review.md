...
| 12. `app-review-monitor-e2e.yml` | GET-only live classification of a candidate `app-review-submit` SHA via `observeReviewStatus`, then the recorded 8e9fbd18-shape rejection fixture via `observe_review_fixture.js`. Proves a pin against real ASC state plus the double-submission rejection class. | Nothing. |
