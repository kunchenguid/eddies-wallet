...
- **Kid surfaces:** child-personal when the configured nickname is known (`Maya's Wallet`, `Hi, Maya`); neutral otherwise (`Your wallet`, `Allowance`, `Balance`). Own reusable strings in `ChildProfileCopy` / `KidCopy`.
