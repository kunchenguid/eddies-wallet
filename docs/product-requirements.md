...
2. The parent pays out allowance and records deposits, withdrawals, loans, and repayments. Creating a weekly allowance or loan payment plan is the parent act that authorizes later local auto-settlement of due occurrences.
...
- The parent is the only role that can create or change the allowance rule, the child profile, or the parent PIN, and the only role that can record a manual money event. On a free local wallet, due occurrences of a parent-created weekly schedule may settle on read without a new parent action; that pass cannot create rules, edit them, or record any other kind of event. A Cloud wallet never auto-writes from this device.
...
- Parent-created weekly allowance and loan schedules, parent-recorded deposits, withdrawals, loans, and repayments, and local apply-on-read settlement of a small due set.
...
2. A service-authoritative wallet whose newest service read could not be used keeps its last accepted snapshot with a **Last updated** time, but that snapshot is read-only. Only a device that itself reports no usable network is described as **offline**; a service that did not answer over a working connection reads as hard to reach, while a service that answered but returned an unusable response reads only as unable to update, never as a connectivity problem. An older response overtaken by a newer accepted revision is a benign no-op that keeps the newer wallet and shows no status, and an attempt cancelled before it finished likewise says nothing. A failure's technical detail is never shown to the child: it is available to a parent inside the Parent area, in a form that carries no account, session, or wallet information.
3. A service-authoritative wallet starts no money, allowance, or profile mutation while known offline or until a successful server read has confirmed the persisted replica revision. The Parent area must name the specific condition blocking those actions and place a reachable recovery control on that block; it must never claim the device is syncing while the same evidence disables the actions. If connectivity is lost after an exact request has been protected and transport has started, the action is clearly marked **Waiting to sync** without changing the accepted balance.
...
4. The calm child status and **Last updated** treatment for service-authoritative reads defined in Journey E.
...
Every manual payment remains parent-triggered. On a free local wallet, a successful read also settles due chain-head installments of a parent-created plan, including today, oldest-first, one accepted entry per save, at most four due occurrences per pass, after any same-day allowance credit. An installment that would overdraft stays scheduled and is retried on a later pass. A larger derived backlog stays on **Pay missed payments**. Cloud wallets never auto-write from this device. There is no timer, background job, or notification, and the kid home never shows waiting, skip, or behind copy.
