...
- `CloudAPIClient` is the public `/v1` contract: capabilities, context, transaction JWS delivery, bootstrap, changes, household import, legacy transition, revision-guarded household commands, and session revoke. Field names, optionality, statuses, and the import hash order are pinned by `EddysWalletTests/Fixtures/cloud-api-contract/v1.json` and asserted in `CloudContractTests`.
...
- The client never grants Cloud. Only a backend-projected `active`/`billing_grace` entitlement in a verified context enables Cloud, and a StoreKit transaction is finished only after that. 202, 4xx, and unreadable responses stay pending or rejected without finishing.
...
- After activation, `CloudWalletRepository` is the accepted authority: every write carries `If-Match: "rev-N"`, an accepted write re-reads `/v1/cloud/changes`, a `409` leaves the action Not recorded and asks the parent to review the latest balance, and an expired entitlement offers local continuation without deleting anything.
- Signing out of a Cloud device revokes the server session and keeps the mirrored wallet usable locally; the destructive erase warning stays only for local-only wallets.

The checked-in StoreKit configuration is a Debug/Test scheme input only: it is selected by the shared scheme's Test and Launch actions and is deliberately **not** a member of the app target's Resources, so it never ships. `CloudDiagnosticsView` is a Debug-only surface that renders what StoreKit resolved (`storekit-product-*`, `storekit-price-*`), and `testDebugStoreKitDiagnosticsProvesTheExactCloudProductsAndPrices` drives it in a real Debug app run to prove the exact product identifiers and the $2.99 / $24.99 display prices. The matching live StoreKit products are configured but have not been submitted, approved, or proven purchasable; Cloud activation remains under development, so no backup, sync, purchase, or TestFlight success is claimed. See [`docs/app-store-configuration.md`](../docs/app-store-configuration.md) for the exact store state and its unproven boundaries.
