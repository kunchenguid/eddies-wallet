...
7. Switch to the configured child's view. Entering child mode must refresh through `/v1/child-view`; confirm the view is read-only and contains no parent money actions. Confirm activity and loan details remain readable. The view uses the service-provided nickname when available and neutral child language otherwise.
