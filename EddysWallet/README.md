...
- After activation, `CloudWalletRepository` is a read-only view of the accepted Cloud authority. It bootstraps the complete household, refreshes through `/v1/cloud/changes`, and keeps the last accepted replica readable offline. Recording money, changing allowance, and editing the child profile are unavailable while Cloud owns the wallet in this slice. An expired entitlement can offer local continuation without deleting anything after the device first catches up with Cloud.
- Signing out of a Cloud device first catches up with Cloud, attempts to revoke the server session, and keeps the mirrored wallet usable locally; the destructive erase warning stays only for local-only wallets.
